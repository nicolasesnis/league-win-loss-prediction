...
def pull_historical_frames(region,  tier, number_matches=10, division='I', queue='RANKED_SOLO_5x5',):
    ...
    entries = get_league_summoners(region, queue, tier, division, pages=1)
        ...
        while not stop:
            ...
            for match_id in match_ids:
                ...
                for participant_puuid in timeframe['metadata']['participants']:
                    ...
                    time.sleep(1)
                    upload_dict_to_s3(ranked_stats, 's3://league-pred-tool/' + path + '/raw_summoner_ranked_data/' + participant_id + '.json')
                    # Mastery data
                    mastery = watcher.champion_mastery.by_summoner(region, participant_id)
                    time.sleep(1)
                    upload_dict_to_s3(mastery, 's3://league-pred-tool/' + path + '/raw_summoner_champion_mastery_data/' + participant_id + '.json')
            stop = True
    return path
